...
function test_TryExecuteViaAccount_Revert() public {
    ...
    assertEq(returnData.length, 1, "Return data should have one element");
    assertEq(keccak256(returnData[0]), keccak256(abi.encodeWithSignature("Error(string)", "Counter: Revert operation")), "Return data should contain revert reason");
}
...
function test_TryExecuteBatchViaAccount_MixedOutcomes() public {
    ...
    assertEq(keccak256(returnData[1]), keccak256(abi.encodeWithSignature("Error(string)", "Counter: Revert operation")), "Second return data should contain revert reason");
...
function test_TryExecuteBatchViaAccount_AllFail() public {
    ...
    assertEq(returnData.length, 3, "Return data should have three elements");
    for (uint256 i = 0; i < returnData.length; i++) {
        assertEq(keccak256(returnData[i]), keccak256(abi.encodeWithSignature("Error(string)", "Counter: Revert operation")), "Return data should contain revert reason");
    }
}

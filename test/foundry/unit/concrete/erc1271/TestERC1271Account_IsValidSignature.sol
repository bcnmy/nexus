...
contract TestERC1271Account_IsValidSignature is Test, SmartAccountTestLab {
    ...
    function test_isValidSignature_EIP712Sign_MockValidator_Success() public {
        ...
        unchecked {
            uint256 vs = uint256(t.s) | (uint256(t.v - 27) << 255);
            signature = abi.encodePacked(t.r, vs, _DOMAIN_SEP_B, t.contents, contentsType, uint16(contentsType.length));
            assertEq(
                ALICE_ACCOUNT.isValidSignature(_toContentsHash(t.contents), abi.encodePacked(address(VALIDATOR_MODULE), signature)),
                bytes4(0x1626ba7e)
            );
        }
    }

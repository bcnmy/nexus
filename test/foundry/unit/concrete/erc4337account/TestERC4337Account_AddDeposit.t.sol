...
contract TestERC4337Account_addDeposit is Test, SmartAccountTestLab {
    ...
    function test_AddDeposit_Success() public {
        uint balanceBefore = ENTRYPOINT.balanceOf(address(account));
        uint256 depositAmount = 1 ether;

        account.addDeposit{value: depositAmount}();

        assertEq(balanceBefore + depositAmount, ENTRYPOINT.balanceOf(address(account)), "Deposit should be added to EntryPoint");
    }
...
function test_AddDeposit_DepositViaHandleOps() public {
    ...
    almostEq(balanceBefore + depositAmount  - (gasUsed * tx.gasprice), ENTRYPOINT.balanceOf(address(account)), maxPercentDelta);
}
...
function test_AddDeposit_BatchDepositViaHandleOps() public {
    ...
    // Using almostEq to compare balances with a tolerance for gas costs
    uint256 maxPercentDelta = 100_000_000_000;
    almostEq(balanceBefore + (depositAmount * 5)  - (gasUsed * tx.gasprice), ENTRYPOINT.balanceOf(address(account)), maxPercentDelta);
}

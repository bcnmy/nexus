...
contract BaseInvariantTest is SmartAccountTestLab {
    ...
    function prepareUserOperation(Vm.Wallet memory signer, Nexus account, Execution[] memory executions) internal view returns (PackedUserOperation[] memory userOps) {
        bytes memory executionCalldata = (executions.length > 1) ? ExecLib.encodeBatch(executions) : ExecLib.encodeSingle(executions[0].target, executions[0].value, executions[0].callData);

...
`;
  ...
  // Only post comment if body is not empty
  if (!body || body.trim().length === 0 || processedBody.trim().length === 0){
  ...
  // Only post comment if body is not empty
  if (!body || body.trim().length === 0 || processedBody.trim().length === 0 || comments.length === 0){
    console.log("Slither report is empty. No comment will be posted.");
    return;
  }

  if(comments.length === 0){
    console.log("No comments found. Creating a new comment.");
    body = "";
  }
